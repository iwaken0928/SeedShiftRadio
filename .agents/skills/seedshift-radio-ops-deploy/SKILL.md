...
- Pipeline / deploy 仕様: `doc/16_パイプライン・コンテナデプロイ仕様書.md`
- 実作業手順: `doc/17_パイプライン・コンテナデプロイ手順書.md`
- 運用、監視、セキュリティ、テスト: `doc/11_運用・監視・セキュリティ・テスト設計書.md`
- DB、config、asset、volume: `doc/04_データ構造設計書.md`
- MusicGen worker 運用: `doc/09_MusicGen連携設計書.md`
- 残タスクと現在地: `doc/15_設計差分棚卸しと段階実装計画.md`
- ローカル起動メモ: `HELP.md`
...
2. `doc/16` と `doc/17` で標準構成を確認してから、現物の pipeline / compose / script を読む
...
7. CI job 名、stage、runner tag、manual 条件を変える場合は `doc/16`, `doc/17`, 必要に応じて `doc/11` を同じ変更で更新する
...
- `doc/16` / `doc/17` と現物の job 名、変数名、port、bind、volume が一致しているか
