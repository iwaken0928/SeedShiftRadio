...
`providers.llm.providers.{providerKey}` は `adapter`, `baseUrl`, `healthPath`, `timeoutMs`, `capabilities`, `defaultModelProfileId` を持つ。`adapter` は `OLLAMA` または `OPENAI_COMPATIBLE` に限定し、`defaultModelProfileId` は profile map の key ではなく Provider へ送る model 名そのものとして使う。既定の Ollama model は `qwen3:8b`、既定 timeout は 20000 ms とする。Ollama は `POST /api/chat` と `GET /api/tags`、OpenAI 互換 Provider は `POST /v1/chat/completions` と `GET /v1/models` を標準 endpoint とする。

LLM endpoint の保存時は default / fallback の参照先、HTTP(S) の `baseUrl`、非空の `healthPath`、adapter、model 名を検証する。`capabilities` には `SCRIPT_GEN` を設定し、既定 profile の `timeoutMs` は 20000 とする。`OPENAI_COMPATIBLE` で認証が必要な場合は `apiKeyRef` を使い、空値または `env:` / `file:` 参照だけを許可する。秘密値の直書きは validation error とする。
