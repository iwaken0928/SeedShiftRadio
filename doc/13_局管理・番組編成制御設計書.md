...
4. `ProgramPlanner` が `ProgramBlock`, `ProgramSlot`, station `compositionPolicy` をもとに block の混ぜ方を確定する
...
- 現行 Preview API は保存済み `StationProgrammingPolicy` と `ProgramTemplate` を評価する。未保存 draft payload の preview は後続拡張とし、実行時 block には採用しない
...
- block 単位の replay 率は `maxReplaySharePercent` を上限とし、同一 source は `cooldownHours` 内で再利用しない
...
現行 Web `/settings` は station 基本情報の `name`, `frequencyMHz`, `genre`, `languagePersonaId`, `defaultVoiceProfileId`, `isActive` と、`StationProgrammingPolicy` の `enabled`, `defaultTemplateId`, `fallbackStrategy`, `planningHorizonMinutes`, `preGeneration`, `replay`, `composition`, `rules` を編集保存できる。`ProgramTemplate` 本体の作成/複製/slot 編集、station 作成/複製、未保存 policy draft に対する preview は後続フェーズで追加する。
