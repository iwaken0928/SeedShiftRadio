...
`targetDurationMs` は計画尺であり、生成 asset の実尺とは区別する。既定の `MUSIC_BREAK` は120000msとし、slot policy の `songForm`, `outroLeadSeconds`, `fadeOutSeconds` で曲構成と終止余白を明示する。生成完了後の実尺は QueueItem 側へ反映し、テンプレートの目標尺自体は書き換えない。
...
7. Web はユーザーが再生開始した `ProgramBlock` を冒頭から末尾まで連続再生し、次の block へ到達した時点で自動再生を解除する
8. 次回の番組再生操作で次の block へ進む
...
`tmpl-night-regular` version 4 の既定 slot は次の順序とする。

| 順序 | role | constraint | 第一候補 | fallback |
|---:|---|---|---|---|
| 1 | `OPENING` | `HARD` | `TALK`, `JINGLE` | `JINGLE` |
| 2 | `MUSIC_BREAK` | `HARD` | `MUSIC_AI` | `MUSIC_LOCAL`, `JINGLE` |
| 3 | `ENDING` | `HARD` | `TALK` | `JINGLE` |

これにより既定番組は冒頭トークの後に音楽を流し、締めトークまでを一つの `ProgramBlock` として扱う。
...
- 管理トップで全局の番組・台本・音声・曲 asset 保有量を比較
- 局と任意の有効な `ProgramTemplate`、番組数、台本・音声、曲を指定したオフエア事前生成
- 局、番組、asset 種別を指定したオフエア事前生成データの削除

現行 Web は管理設定をカテゴリー分割し、`/settings/stations` で station 基本情報の `name`, `frequencyMHz`, `genre`, `languagePersonaId`, `defaultVoiceProfileId`, `isActive` の新規作成/複製/編集保存を扱い、`/settings/programming` で `StationProgrammingPolicy` の `enabled`, `defaultTemplateId`, `fallbackStrategy`, `planningHorizonMinutes`, `preGeneration`, `replay`, `composition`, `rules` を編集保存する。station 複製は局基本情報だけを新規 draft へ引き継ぎ、`id` と `frequencyMHz` は新規候補へ補正し、`STATION` scope template を指す `defaultProgramTemplateId` と policy/rules は自動複製しない。`ProgramTemplate` は create mode と duplicate mode を持ち、`scope`, `stationId`, `name`, `targetDurationMinutes`, `planningHorizonMinutes`, `isActive`, `fallbackTemplateId`, `editorialPolicy`, `slots` を編集保存できる。`editorialPolicy` と `slotPolicy` は JSON object editor とし、slot の並び順がそのまま保存順となる。保存で `400 VALIDATION_ERROR` や `409 CONFLICT` が返った場合は draft を保持したまま message と field error を表示し、修正または再取得後に再試行できる。Preview は保存済み `StationProgrammingPolicy` / `ProgramTemplate` に加えて、現在編集中の未保存 policy/template draft を request payload として含めて評価できる。policy 未保存局では station の既定 template から既定 policy を合成して評価し、slot ごとの provider / letter 条件で fallback が発生した時は `fallbackApplied` と warning に反映する。runtime はここに加えて `composition.maxConsecutiveTalkSegments`, `composition.musicBreakIntervalMinutes`, `composition.letterPriorityBoostThreshold` を `SOFT` slot 解決へ反映するが、preview 結果を runtime block や DB 正本へそのまま反映しない。

`/settings` は `GET /api/management/dashboard` を使う管理トップとし、`/settings/content` は局別 `StationContentInventory`、番組別コンテンツ詳細、事前生成フォーム、事前生成データ削除フォームを提供する。
局を選ぶと `GET /api/management/stations/{stationId}/content/programs` から直近 100 番組を取得し、番組 block、template、予定時刻、segment、asset 種別、生成元、状態、容量を確認できる。
番組単位の削除操作は `purpose=PRE_GENERATION` の番組にだけ表示し、ライブ番組からの誤削除を UI と Server の両方で拒否する。
事前生成で `programTemplateId` を省略した場合は保存済み policy / rule から現在時刻に合う template を解決し、指定時は同一局または `GLOBAL` scope の有効 template だけを使う。
`targetProgramCount` は 1 から 10 とし、作成した block は `purpose=PRE_GENERATION` のオフエア `playout_session` に紐づける。
この session はライブの最新 session、現在 block、再生 queue の正本にならず、MusicGen 完了時にもライブ SSE を更新しない。
削除フォームは `SCRIPT`, `AUDIO`, `MUSIC` を複数選択し、確認後に指定局または指定番組の `purpose=PRE_GENERATION` かつ archive 対象外の生成 asset を物理削除する。対応する `QueueItem.assetId` と `assetUrl` を解除して再計画可能な状態へ戻し、共有 path がライブ側などの削除対象外 asset から参照されている場合は実ファイルを保持する。ライブ session、番組 block、Provider job、放送 archive、監査履歴は保持し、削除後に局別台帳と番組詳細を再取得する。過去の eviction tombstone も同じ選択範囲に含め、実行中 MusicGen が遅れて完了した場合も未参照 asset を破棄して、管理画面から削除したデータが asset record として残らないようにする。
...
- `GET /api/management/dashboard`, `GET /api/management/stations/{stationId}/content`, `GET /api/management/stations/{stationId}/content/programs`, `POST /api/management/stations/{stationId}/pre-generations`, `POST /api/management/stations/{stationId}/content/deletions`, `POST /api/management/stations/{stationId}/programs/{programBlockId}/content/deletions` を管理トークン保護し、局別コンテンツ台帳、番組別詳細、オフエア事前生成、安全な物理削除に使う
- `pre_generation_request` は受付状態と materialize 件数だけを保持し、生成入力本文は `provider_job` と同様に保持しない
...
- 事前生成の失敗 response は、特定できた Provider 起因なら共通 Provider error code、
  asset storage や DB などの内部失敗なら `PRE_GENERATION_FAILED` を返す。
  いずれも例外本文、絶対 path、Provider 応答本文を返さない
- 事前生成で `LETTER` slot を materialize しても、ライブ放送への採用状態遷移やレター消費を行わない
