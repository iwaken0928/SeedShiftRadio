...
- `GET /api/letters`, `GET /api/letters/{id}`, `POST /api/letters/{id}/status`, `POST /api/letters/{id}/reply`, `GET /api/play-history`, `GET /api/play-history/{id}` は `X-Admin-Token` 前提とし、公開側は `POST /api/letters` のみを開ける
