...
`/settings` は `GET /api/management/dashboard` を使う管理トップとし、`/settings/content` は局別 `StationContentInventory` と事前生成フォームを提供する。
...
- `GET /api/management/dashboard`, `GET /api/management/stations/{stationId}/content`, `POST /api/management/stations/{stationId}/pre-generations` を管理トークン保護し、局別コンテンツ台帳とオフエア事前生成に使う
