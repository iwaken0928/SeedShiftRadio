...
現行 runtime は `composition` の first step として、`SOFT` slot に対して `maxConsecutiveTalkSegments`, `musicBreakIntervalMinutes`, `letterPriorityBoostThreshold` を反映する。未採用レター件数が閾値以上なら `LETTER` 候補を優先し、連続 talk 上限や music break 間隔に達した場合は `MUSIC_LOCAL` / `MUSIC_AI` / `JINGLE` の候補があれば優先する。`targetSegmentShares` と `allowSoftFallbackRetiming` は未実装のまま残す。
