...
- 全初期化は `reset-production-data` manual job に限定し、GitLab の確認ダイアログ、対象 volume 検証、サービス停止を通した後だけ DB schema と永続データを削除する
...
本番データの全初期化は通常の asset 削除 API と分離し、GitLab manual job `reset-production-data` だけで行う。対象は PostgreSQL の `public` schema、`SEEDSHIFT_RADIO_DATA_VOLUME` 配下の config / asset、設定済み `MUSICGEN_DATA_VOLUME` 配下の worker data とする。初期化後は Flyway migration と application deploy を再実行して初期 seed 状態へ戻す。個別局・個別番組の削除にはこの経路を使わない。
