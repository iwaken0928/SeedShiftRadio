...
| Local AI Services | LLM, TTS, MusicGen, 必要時 STT | Ollama, VOICEVOX, Irodori-TTS-Server, FastAPI Worker |
...
flowchart LR
    ...
    TTS["TTS Service (VOICEVOX / Irodori-TTS-Server etc.)"]
...
| `management-service` | 管理トップの全体集約、局別コンテンツ台帳、ライブから分離したオフエア事前生成受付 |
...
- `/infra/compose`: PostgreSQL, Ollama, VOICEVOX, Irodori-TTS-Server などのローカル起動定義
...
1. `management-service` が局、有効な番組テンプレート、生成数、生成対象を検証する
2. `purpose=PRE_GENERATION` の専用 `playout_session` と受付 record を作る
3. `programming-service` が保存済み policy / rule または指定テンプレートから番組 block を解決する
4. `job-service` が台本・TTS を materialize し、MusicGen を既存の非同期生成へ投入する
5. `management-service` が局別の番組・asset 保有量と受付状態を集約する

この経路はライブ session の最新検索、radio status、現在番組、再生 queue、SSE、レター採用状態を変更しない。

## 10. 主要な設計判断

- Web UI は `SSR shell + Client Components` のハイブリッド構成とする
- `Server-side TTS` を Web の標準とし、`Client-side TTS` は契約のみ先行定義する
- Irodori-TTS は server-side TTS の高品質候補として `Irodori-TTS-Server` 経由で扱い、Java Server から Python CLI を直接実行しない
