...
- Preview API は保存済み `StationProgrammingPolicy` と `ProgramTemplate` を基本としつつ、管理画面から渡した未保存 `StationProgrammingPolicy` draft と `ProgramTemplate` draft を request payload として評価できる。draft preview は実行時 block に採用せず、DB 保存もしない
