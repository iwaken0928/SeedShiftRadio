...
現行 runtime では `compositionPolicy` のうち `maxConsecutiveTalkSegments`, `musicBreakIntervalMinutes`, `letterPriorityBoostThreshold`, `allowSoftFallbackRetiming` を `SOFT` slot の解決へ反映する。未採用レター件数が閾値以上なら `LETTER` 候補を優先し、連続 talk 上限や music break 間隔を超える場合は `MUSIC_LOCAL` / `MUSIC_AI` / `JINGLE` が候補にあればそちらを優先する。`allowSoftFallbackRetiming=false` の場合はこれらの composition 起因の優先変更を行わず、candidate / fallback の通常解決順を維持する。`targetSegmentShares` は引き続き後続実装対象とする。
