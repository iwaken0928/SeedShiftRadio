...
現行 Web `/settings` は station 基本情報の `name`, `frequencyMHz`, `genre`, `languagePersonaId`, `defaultVoiceProfileId`, `isActive` の新規作成/複製/編集保存と、`StationProgrammingPolicy` の `enabled`, `defaultTemplateId`, `fallbackStrategy`, `planningHorizonMinutes`, `preGeneration`, `replay`, `composition`, `rules` を編集保存できる。station 複製は局基本情報だけを新規 draft へ引き継ぎ、`id` と `frequencyMHz` は新規候補へ補正し、`STATION` scope template を指す `defaultProgramTemplateId` と policy/rules は自動複製しない。`ProgramTemplate` 本体の作成/複製/slot 編集と未保存 policy draft に対する preview は後続フェーズで追加する。
