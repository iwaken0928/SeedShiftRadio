...
`reference_voice_ref` は Provider が管理する voice id、または許可済み data root から解決する安全な相対参照に限定する。絶対 path、URL、`..` による親 directory traversal は拒否し、指定時は空でない `consent_policy_ref` を必須とする。参照音声の実 path、参照元の個人名、同意文書の機微情報は API response、SSE、標準ログへ出さない。

Irodori-TTS では `engine_type=IRODORI_TTS`, `speaker_key=<Irodori voice id>`, `style_key=<style preset>`, `speed=<OpenAI互換 speed>` として識別する。`provider_options`、`reference_voice_ref`、`consent_policy_ref` を TTS request と生成 asset metadata へ反映する処理は `P0-02b` の後続実装とし、現段階では安全な永続化と station 割当境界を先に固定する。
