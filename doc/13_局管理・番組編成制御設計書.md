...
現行 runtime は `composition` の first step として、`SOFT` slot に対して `maxConsecutiveTalkSegments`, `musicBreakIntervalMinutes`, `letterPriorityBoostThreshold`, `allowSoftFallbackRetiming` を反映する。未採用レター件数が閾値以上なら `LETTER` 候補を優先し、連続 talk 上限や music break 間隔に達した場合は `MUSIC_LOCAL` / `MUSIC_AI` / `JINGLE` の候補があれば優先する。`allowSoftFallbackRetiming=false` の場合は composition による優先変更を行わず、candidate / fallback の通常順で解決する。`targetSegmentShares` は未実装のまま残す。
