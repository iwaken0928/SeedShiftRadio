...
- `resumePlayback=true` の場合も `POST /api/radio/tune` の初回応答は `PREPARING` とし、after-commit の warmup 完了後に client が `/api/radio/play` または `SEGMENT_STARTED` で `PLAYING` へ進める
