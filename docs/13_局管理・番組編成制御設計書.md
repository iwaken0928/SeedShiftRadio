...
- 局と asset 種別を指定したオフエア事前生成 payload の削除
...
`/settings` は `GET /api/management/dashboard` を使う管理トップとし、`/settings/content` は局別 `StationContentInventory`、事前生成フォーム、事前生成 payload 削除フォームを提供する。
...
削除フォームは `SCRIPT`, `AUDIO`, `MUSIC` を複数選択し、確認後に指定局の `purpose=PRE_GENERATION` かつ archive 対象外の payload だけを削除する。ライブ session、番組 block、queue、Provider job、asset record、監査履歴は保持し、削除後に局別台帳を再取得する。
...
- `GET /api/management/dashboard`, `GET /api/management/stations/{stationId}/content`, `POST /api/management/stations/{stationId}/pre-generations`, `POST /api/management/stations/{stationId}/content/deletions` を管理トークン保護し、局別コンテンツ台帳、オフエア事前生成、安全な payload 削除に使う
