...
| `content_origin` | varchar | `LIVE_GEN`, `CACHE_REUSED`, `ARCHIVE_REPLAY`, `LOCAL_LIBRARY`, `PLACEHOLDER` |
...
- `last_accessed_at`, `reuse_count`, `expires_at`, `archive_eligible` は retention/eviction と archive 昇格のための予約カラムであり、現時点では定期削除や更新ジョブは未実装
...
この eviction 順序は設計上の定義であり、現時点の実装では `generated_asset.cache_key` による再利用までが先行している。`last_accessed_at` の更新、`reuse_count` の増分、`expires_at` に基づく自動削除、`cleanupBatchSize` を使う定期ジョブは未実装で、Phase 2 の残課題として扱う。
