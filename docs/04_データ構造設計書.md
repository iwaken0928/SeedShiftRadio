...
局別コンテンツの手動削除も同じ tombstone 規則を使い、`playout_session.purpose=PRE_GENERATION`、指定局、指定 asset type、`byte_size > 0`、`archive_eligible=false` をすべて満たす record だけを対象とする。削除理由は `station-content-delete` とし、`program_block`, `queue_item`, `provider_job`, `generated_asset`, `operational_event_log`, `broadcast_archive` の履歴行は物理削除しない。削除操作自体は `station_content.deleted` として局 ID、成功・失敗件数、回収容量だけを監査記録する。
