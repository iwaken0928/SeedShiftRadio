...
`QueueItem.durationMs` は `PLANNED` / `GENERATING` では slot の目標尺とし、MusicGen が `SUCCEEDED` になった時点で登録対象 WAV の frame 数と frame rate から測った実尺へ置き換える。WAV を解析できない場合だけ worker の `durationSec`、さらに値がない場合は計画尺へフォールバックする。cache hit でも `generated_asset.metadata.duration` を引き継ぐ。これにより、Provider profile の上限や生成結果によって目標尺と実尺が異なっても、`READY` 以降のキュー表示と再生進捗が実音源からずれないようにする。
...
既定テンプレートの中央 `MUSIC_BREAK` は120秒を目標とする。生成 prompt は目標尺の20秒前までにアウトロへ入り、終止和音を解決し、最後の6秒で自然にフェードするよう要求する。単に末尾サンプルを小さくするだけでは曲として完結したと扱わず、ボーカル、フレーズ、持続音を時間境界で切らないことを品質条件とする。
