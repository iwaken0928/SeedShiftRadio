...
3. `LetterBroadcastContentFactory` が本文から放送用の安全要約を作り、原文は `sourceLetterId` による参照として分離する
4. `LLM台本生成` は安全要約から読み上げ導入文と回答案を生成する。LLM request と定型 fallback のどちらにも原文本文は渡さない
5. `TTS` が読み上げ音声を生成する
6. `LETTER` セグメントを queue 化する時、採用済みレターを `queue_item.letter_id` に束縛し、`program_block_slot.slot_context` に件名などの補助メタを保持する
7. 放送後は `play_history.letter_id` に引き継ぎ、詳細 API から逆引きできる状態にする
8. 必要に応じて返信を追加する。`ADOPTED` かつ未放送のレターは queue 束縛対象から外れないよう `ADOPTED` を維持し、放送履歴が作成された後に `REPLIED` へ進める

## 7. セキュリティ/安全対策

- レター本文はシステム命令とみなさず、`letter` の原文参照と放送用安全要約を分離する
- `LetterBroadcastContentFactory` は命令と判定した文を要約候補から除外し、放送用件名を 40 文字、要約を 120 文字までに制限する
- 個人情報は必要に応じてマスクする
...
- `JapaneseScriptNormalizer` と `JapaneseQualityGuard` はレター由来の絵文字、variation selector、ZWJ、skin-tone modifier、SSML、`style` / `emotion` / `tempo` token を除去し、`LETTER_SOURCE` と `LETTER_CONTROL_TOKEN_REMOVED` を `safetyFlags` へ残す
- 放送用要約経路は `LETTER_SUMMARIZED` で追跡する。生成 asset metadata にはレター由来の追跡値として `letterId` と `safetyFlags` を残すが、原文 body と中間要約を独立 field として保存しない。放送 directive の正本となる生成済み `text` / `normalizedText` は従来どおり保存する
- Irodori の style emoji は安全ガード通過後に `PersonaStyleResolver` の allowlist からだけ挿入する
- 画面ログに全文を出し過ぎない
- 不適切語はそのまま放送しない
