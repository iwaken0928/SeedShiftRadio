...
- DB 名とユーザーは標準で `seedshift_radio` / `seedshift` を使い、既存 PostgreSQL の構成が異なる場合だけデプロイ環境の環境変数で上書きする。DB パスワードは保護された secret store から渡し、リポジトリには実値を置かない
- Flyway migration は application deploy 前に適用し、失敗した場合は Server / Web container の更新へ進めない
- 全初期化は通常のデプロイと分離し、バックアップ、対象 volume の確認、サービス停止を通した後だけ DB schema と永続データを削除する
...
- MusicGen worker の `/data` を永続化する場合は Podman managed volume または絶対 host path を mount する。未設定時は worker container 内の一時領域で、container 再作成時に生成物は残らない
- host path を使う場合は、Runner host 上で directory、実行ユーザーの書き込み権限、backup 方針を事前に準備し、絶対パスや秘密値を標準ログへ過剰に出さない
- production の ACE-Step 接続は `providers.musicGen.providers.ace-step.apiKeyRef=env:ACESTEP_API_KEY` とし、同じ非空の `ACESTEP_API_KEY` を ACE-Step container と Server container の両方へ protected variable から注入する。秘密値を `config.json` へ直書きしない
...
erDiagram
    ...
    station ||--o{ pre_generation_request : prepares
    pre_generation_request ||--|| playout_session : materializes
...
| `station_id` | varchar | `scope=STATION` の所属局。`GLOBAL` では `null` |
...
| `reference_voice_ref` | varchar | 参照音声を使う engine 用の provider voice id または安全な相対参照 |
| `consent_policy_ref` | varchar | 参照音声・声質利用の同意、ライセンス、禁止事項への参照。`reference_voice_ref` 指定時は必須 |

`VoiceProfileEntity` の `scope`, `stationId`, `providerKey`, `providerOptions`, `referenceVoiceRef`, `consentPolicyRef` は、それぞれ `voice_profile.scope`, `station_id`, `provider_key`, `provider_options`, `reference_voice_ref`, `consent_policy_ref` に永続化する。

`scope=GLOBAL` では `station_id` を `null`、`scope=STATION` では `station_id` を必須とする。station の `default_voice_profile_id` には `GLOBAL` または同じ station に属する `STATION` profile だけを割り当て、他局の profile は拒否する。

`V12__voice_profile_metadata.sql` は既存 profile を既定で `GLOBAL` として移行する。seed の `voice-night-main` は `station-night` だけが参照している場合に限り `STATION` へ移し、追加済み station と共有されている既存環境では参照整合を壊さないよう `GLOBAL` を維持する。

`reference_voice_ref` は Provider が管理する voice id、または許可済み data root から解決する安全な相対参照に限定する。絶対 path、URL、`..` による親 directory traversal は拒否し、指定時は空でない `consent_policy_ref` を必須とする。参照音声の実 path、参照元の個人名、同意文書の機微情報は API response、SSE、標準ログへ出さず、生成 asset metadata には `referenceVoiceHash` と `consentPolicyHash` だけを保存する。

Irodori-TTS では `engine_type=IRODORI_TTS`, `speaker_key=<Irodori voice id>`, `style_key=<style preset>`, `speed=<OpenAI互換 speed>` として識別する。`TtsRuntimeProfileResolver` は生成直前に session から station と既定 `VoiceProfileEntity` を再解決し、`GLOBAL` または同一 station の `STATION` scope、`speed` の `0.25` から `4.0`、参照音声の安全性、`consent_policy_ref` の存在を再検証する。検証済み値だけを `TtsRuntimeProfile` として TTS adapter へ渡す。
...
| `purpose` | varchar | `LIVE` または `PRE_GENERATION` |
...
局別コンテンツ集計では `generated_asset.queue_item_id -> queue_item.program_block_id -> program_block.station_id` を正規の追跡経路とする。
`generated_asset.metadata.stationId` の有無や値だけに依存して集計しない。
保有件数と容量は payload が残る `byte_size > 0` の record を対象とし、eviction 済み record は履歴として残しても保有量へ含めない。

### 6.7.1 `pre_generation_request`

| カラム | 型 | 説明 |
|---|---|---|
| `id` | varchar | `pregen-*` の request ID |
| `station_id` | varchar | 事前生成対象局 |
| `session_id` | varchar | `purpose=PRE_GENERATION` のオフエア session |
| `program_template_id` | varchar nullable | 指定テンプレート。`NULL` は局ルールから自動選択 |
| `target_program_count` | integer | 生成する番組数。1 から 10 |
| `include_speech` | boolean | script / TTS asset を生成するか |
| `include_music` | boolean | local music / MusicGen asset を生成するか |
| `status` | varchar | `QUEUED`, `RUNNING`, `MATERIALIZED`, `FAILED` |
| `materialized_program_count` | integer | 作成済み `program_block` 数 |
| `materialized_segment_count` | integer | 作成済み `queue_item` 数 |
| `queued_music_count` | integer | `GenerateMusicJob` へ投入した件数 |
| `error_code` | varchar nullable | Provider 起因の共通 error code、または内部失敗の `PRE_GENERATION_FAILED` |
| `requested_at` | timestamptz | 受付日時 |
| `started_at` | timestamptz nullable | job 開始日時 |
| `completed_at` | timestamptz nullable | materialize または失敗確定日時 |
| `updated_at` | timestamptz | 更新日時 |

`pre_generation_request` は生成入力本文を保持しない。
Provider 起因を特定できる場合は `PROVIDER_TIMEOUT`, `PROVIDER_UNREACHABLE`,
`PROVIDER_BAD_RESPONSE`, `PROVIDER_RESOURCE_EXHAUSTED` などの共通 error code を保持する。
asset storage や DB など Server 内部処理の失敗は `PRE_GENERATION_FAILED` とし、
例外本文、絶対 path、Provider 応答本文は保持しない。
`session_id` は一意とし、`target_program_count` は 1 から 10、`include_speech` と `include_music` は少なくとも一方を `true` にする。
`playout_session.purpose=PRE_GENERATION` は script / TTS / MusicGen が既存の session、block、queue item、asset 契約を再利用するためのオフエア作業単位であり、`RadioService` の最新ライブ session 検索対象から除外する。
事前生成で assigned ID を持つ `queue_item` を一括保存した後に台本・音声 asset を反映する場合は、
repository が返す managed entity を後続処理へ渡す。
merge 元 entity の lifecycle field が未反映でも `created_at` / `updated_at` の NOT NULL 制約を
破らないよう、更新 callback でも欠落した `created_at` を補完する。
...
既定の `tmpl-night-regular` は `V15__default_talk_music_program.sql` 以降、`OPENING(TALK) -> MUSIC_BREAK(MUSIC_AI) -> ENDING(TALK)` の 3 slot とする。`V17__complete_music_break_duration.sql` 以降の `MUSIC_BREAK` 目標尺は120秒で、`slot_policy` に `songForm`, `outroLeadSeconds=20`, `fadeOutSeconds=6` を保持する。
中央の音楽 slot は `HARD`、`MUSIC_AI` を第一候補、`MUSIC_LOCAL`, `JINGLE` を fallback とし、既存環境も Flyway migration で同じ編成へ更新する。
ローカル音源が未登録の skeleton 環境では、`MUSIC_LOCAL_PLACEHOLDER` はジングル用の短い cue と分け、和音、ベース、旋律、リズムを持つ音楽用 WAV を生成する。
...
### 6.18 `operational_event_log`

| カラム | 型 | 説明 |
|---|---|---|
| `id` | varchar | 運用ログID |
| `level` | varchar | `INFO`, `WARN`, `ERROR` |
| `category` | varchar | `PROVIDER_JOB`, `PRE_GENERATION` などの分類 |
| `event_type` | varchar | `provider.job.failed`, `pre_generation.failed` など |
| `source_id` | varchar | request / job ID。外部キーは持たない |
| `correlation_id` | varchar | 一連の処理を追跡する相関ID |
| `provider_type` | varchar | `LLM`, `TTS`, `MUSIC` |
| `provider_key` | varchar | 設定上の Provider key |
| `error_code` | varchar | 分類済み共通 error code |
| `message` | varchar(500) | 分類済み code から生成する安全な説明 |
| `occurred_at` | timestamptz | 発生時刻 |

Provider job と事前生成 request への外部キーは持たせず、生成 transaction が rollback した場合も診断 record を独立 transaction で確定する。
`occurred_at DESC` と category / level の時系列検索用 index を持つ。
保持期間は 30 日とし、新しい event の保存時に期限切れ record を削除する。
prompt、lyrics、台本本文、Provider 応答本文、letter body、radioName、API key、管理トークン、例外 message は保存しない。
    ...
    "tts": {
      ...
      "providers": {
        "irodori": {
          ...
          "healthPath": "/health",
          "timeoutMs": 180000,
          "capabilities": ["TTS_GEN", "OPENAI_AUDIO_SPEECH", "IRODORI_TTS", "VOICE_CLONE", "STYLE_EMOJI", "LONG_TEXT_CHUNKING", "CHUNK_SSE_AVAILABLE"],
          "adapter": "IRODORI_OPENAI_TTS",
          "apiKeyRef": "env:IRODORI_TTS_API_KEY",
          "defaultModelProfileId": "irodori-tts"
        },
...
production compose では Server の作業ディレクトリを永続 volume の mount point
`/var/lib/seedshift-radio` に固定する。
このため `dataRoot=./data` の相対パスは `/var/lib/seedshift-radio/data` として解決され、
script / audio / music payload は container image の `/app` ではなく永続 volume へ保存される。
production で絶対パスを設定する場合も `/var/lib/seedshift-radio` 配下に限定する。

本番データの全初期化は通常の asset 削除 API と分離し、運用者がバックアップと対象を明示確認できる専用手順だけで行う。対象は PostgreSQL の `public` schema、Server の config / asset 永続領域、MusicGen worker の永続領域とする。初期化後は Flyway migration と application deploy を再実行して初期 seed 状態へ戻す。個別局・個別番組の削除にはこの経路を使わない。
...
`features.jobExecution` は `singleGpuMode`, `resourceGroup`, `requireAceStepCpuOffload` と `manual` / `automatic` の `JobExecutionPolicy` を持つ。各 policy は `waitStrategy`, `resourceWaitTimeoutSeconds`, `providerIdleTimeoutSeconds`, `modelLoadTimeoutSeconds`, `jobTimeoutSeconds`, `pollIntervalMillis`, `unloadOllamaBeforeMusic`, `waitForAceStepIdleBeforeLlm` を持つ。設定の正本は `config.json` であり、ジョブ開始時に読み込んだ policy をそのジョブの Provider 呼び出しへ固定して適用する。

`providers.llm.providers.{providerKey}` は `adapter`, `baseUrl`, `healthPath`, `timeoutMs`, `capabilities`, `defaultModelProfileId` を持つ。`adapter` は `OLLAMA` または `OPENAI_COMPATIBLE` に限定し、`defaultModelProfileId` は profile map の key ではなく Provider へ送る model 名そのものとして使う。既定の Ollama model は `qwen3:8b`、既定 timeout はモデルロードを含む初回生成を考慮して 120000 ms とする。Ollama は `POST /api/chat` と `GET /api/tags`、OpenAI 互換 Provider は `POST /v1/chat/completions` と `GET /v1/models` を標準 endpoint とする。

LLM endpoint の保存時は default / fallback の参照先、HTTP(S) の `baseUrl`、非空の `healthPath`、adapter、model 名を検証する。`capabilities` には `SCRIPT_GEN` を設定し、既定 profile の `timeoutMs` は 120000 とする。`OPENAI_COMPATIBLE` で認証が必要な場合は `apiKeyRef` を使い、空値または `env:` / `file:` 参照だけを許可する。秘密値の直書きは validation error とする。

`providers.tts.providers.{providerKey}` は `adapter` として `VOICEVOX` または `IRODORI_OPENAI_TTS` を持てる。Irodori adapter は OpenAI 互換の `/v1/audio/speech` を使い、Provider endpoint には接続先と認証参照だけを置く。Irodori の声質・style・chunking・response format など station/voice ごとの設定は `voice_profile.provider_options` に置く。実行時は `VoiceProfileEntity.providerKey` を既定 Provider より先に解決し、設定済み fallback を後続候補とする。`responseFormat` は現行 asset 契約に合わせて `wav` のみ許可し、`irodori` object は `num_steps`, `seed`, `cfg_scale_text`, `cfg_scale_speaker`, `t_schedule_mode`, `sway_coeff`, `chunking_enabled`, `chunk_min_chars` の安全な allowlist だけを request へ反映する。`lora_adapter`, `ref_wav`、未知 field は転送しない。`CHUNK_SSE_AVAILABLE` は upstream 能力だけを示し、adapter は `stream_format=sse` を使わず完成 WAV を保存する。
...
台本の `generated_asset.metadata` は再生 directive の復元に必要な生成済み `text`, `normalizedText`, `pronunciationHints`, `pauseHints`, `emotion`, `tempo`, `personaRef`, `voiceHint`, `safetyFlags` と、`providerKey`, `providerJobId`, `textHash`, `promptHash` などを保存する。入力 prompt、レター本文、raw Provider response、`apiKeyRef` の解決値は保存しない。実 Provider の adapter と model は `provider_fingerprint` で識別する。LLM fallback では Provider 試行ごとに `provider_job` を作り、同じ論理生成要求の `correlation_id` を継承する。成功した実 Provider の script asset はその成功 job を参照する。Provider chain 全体の失敗後に `TemplateScriptProvider` へ縮退する場合は `providerKey=template-script` の別 job と asset を作り、失敗した外部 Provider job を `SUCCEEDED` へ書き換えない。

TTS 音声の `generated_asset.metadata` には `normalizedTextHash`, `voiceHintHash`, `voiceProfileId`, `engineType`, `providerKey`, `adapter`, `model`, `responseFormat`, `durationMs`, `stylePreset`, `referenceVoiceHash`, `consentPolicyHash`, `scriptAssetId`, `scriptProviderJobId` のような短い再現・追跡用メタだけを保存する。Irodori の参照音声ファイル path、参照元の個人名、生の `voiceHint`、レター本文、radioName、API key は保存しない。
...
現行 script cache key は Provider identity、reuse scope と partition、segment / slot、program block / slot、personality、`promptHash` を正規化入力にする。現行 TTS cache key は Provider identity、reuse scope と partition、`normalizedTextHash`, `voiceHintHash`、VoiceProfile の有効設定 fingerprint、fallback 分類を正規化入力にする。`LETTER` 由来は状態変化と本文隔離を優先し、script / TTS とも通常 cache の横断再利用を行わない。
...
現時点の実装では script、TTS、MusicGen が `generated_asset.cache_key` を使って cache-first 再利用する。cache hit は Provider 呼び出しを省略し、既存 payload を現 queue item 用の asset record へ clone する。監査上は論理生成 job を `external_ref=cache-hit:{sourceAssetId}` として成功記録し、clone asset はその job と元 asset ID を metadata から追跡できる。`last_accessed_at` の更新、`reuse_count` の増分、`expires_at` と種別別 size cap に基づく payload eviction、`cleanupBatchSize` を使う定期 job まで対応する。`queue_item.asset_id` や `broadcast_archive` の FK を壊さないため、eviction は DB record を物理削除せず、payload file を削除して cache 対象から外す。同一 `storage_path` を複数 record が共有している場合は、最後の active record になるまで実ファイルは削除しない。`archive_eligible=true` の asset は archive 昇格と再放送正本の保持方針を優先し、この job の候補から外す。

局別・番組別コンテンツの手動削除は自動 eviction と分離する。`playout_session.purpose=PRE_GENERATION`、指定局または指定 `program_block`、指定 asset type、`archive_eligible=false` を満たす `generated_asset` を対象とし、過去の手動削除で残った `byte_size=0` の tombstone も含めて物理削除する。対象を参照する `queue_item.asset_id` / `asset_url` は解除し、status を `PLANNED`、`content_origin` を `DELETED` にする。`program_block`, `queue_item`, `provider_job`, `operational_event_log`, `broadcast_archive` は保持する。同じ `storage_path` に対象外の active record があれば実ファイルを保持し、最後の active 参照を削除する時だけ payload を削除する。非同期 MusicGen が削除後に完了しても、queue status の競合を検出して遅れて登録された未参照 asset を破棄する。操作は `station_content.deleted` として局 ID、成功・失敗件数、回収容量だけを監査記録する。
